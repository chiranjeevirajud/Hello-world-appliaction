node(){
    stage('GitClone-SourceCode'){
            sh """
                rm -rf *
            """
            git branch: 'main', credentialsId: 'gitcred-1', url: 'https://github.com/chiranjeevirajud/Hello-world-appliaction.git'
    }
    ...
    node('docker-1'){
        stage('Docker Container Creation'){
            sh """
                ...
                docker run -it -p 8081:80 --name HelloWorld-App 502203921648.dkr.ecr.ap-south-1.amazonaws.com/javaapp:${BUILD_NUMBER}
            """
        }
    }
}
